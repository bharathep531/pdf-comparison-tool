...
public class PDFAutomation {
	...
	private final static Logger logger = Logger.getLogger(PDFAutomation.class.getName());
	...
	public PDFAutomation(){
		this.bTrimWhiteSpace = false;
   ...
   */
	private String getPDFText(String file, int startPage, int endPage) throws IOException{
		...
		System.out.println("PDF new Text before trimming : " + txt.length());
   ...
   */	
	...
	private boolean comparepdfFilesWithTextMode(String file1, String file2, int startPage, int endPage) throws IOException{
		...
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>11>>>"+file1Txt.length());
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>22>>>"+file2Txt.length());
